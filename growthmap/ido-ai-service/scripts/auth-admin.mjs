...
const client = createIdentityToolkitClient({ projectId, getAccessToken: createAdminTokenProvider(sa).getAccessToken });
...
try {
  switch (cmd) {
    ...
    default:
      console.error('用法：node scripts/auth-admin.mjs <status|configure|list|set-password <email>|verify-email <email>>');
      process.exit(2);
  }
} catch (e) {
