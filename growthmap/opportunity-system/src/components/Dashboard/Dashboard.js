...
import GrowthGapDashboard from './GrowthGapDashboard';
import AiRankPanel from '../ai/AiRankPanel';
import { useNav } from '../../contexts/NavContext';
import { isShortlisted } from '../../utils/opportunityStatus';
export default function Dashboard() {
  const { state, dispatch } = useOpportunity();
  const { goToolLibrary, goCheck, goHandoff } = useNav();
  const { opportunities } = state;
  const shortlistedCount = opportunities.filter(isShortlisted).length;
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        {/* 成長差距儀表（MOD-01/08，讀第二堂） */}
        <GrowthGapDashboard />

        {/* 進度列 */}
        <div className="glass-card rounded-2xl p-6">
          <ProgressBar count={shortlistedCount} total={opportunities.length} />
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            className="inline-flex items-center px-5 py-2.5 glass-card text-gray-700 font-semibold rounded-lg transition-colors hover:bg-white/70 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2"
          >
