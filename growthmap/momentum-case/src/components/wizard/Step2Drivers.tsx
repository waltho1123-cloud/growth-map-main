...
export default function Step2Drivers() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-2xl font-bold text-gray-800">驅動因子設定</h2>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200">
              ...
              <th className="text-center px-6 py-4 text-sm font-semibold text-gray-600 w-48">
                歷史貢獻度 (%)
              </th>
              <th className="text-center px-6 py-4 text-sm font-semibold text-gray-600 w-48">
                未來假設 (%)
          ...
          <tbody>
            {drivers.length === 0 ? (
              ...
              drivers.map((driver, idx) => (
                <tr
                  key={driver.id}
                  className={`border-b border-gray-100 ${idx % 2 === 0 ? 'bg-white/30' : ''}`}
                >
