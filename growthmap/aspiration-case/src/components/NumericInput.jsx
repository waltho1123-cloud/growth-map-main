...
export function NumericInput({ value, onValueChange, onFocus, onBlur, onChange, ...rest }) {
  const isEmpty = (v) => v === 0 || v == null || Number.isNaN(v);
  const [display, setDisplay] = useState(isEmpty(value) ? '' : String(value));
  const focusedRef = useRef(false);

  useEffect(() => {
    if (!focusedRef.current) {
      setDisplay(isEmpty(value) ? '' : String(value));
    }
  }, [value]);
